	...
	public void run() {
		...
		try {
			...
			for (int i = 0; i < 6; i++) {
				...
				handshakeSend(authenticationKey, key, 20, 300000, clientIP, PORT);
		...
		while (running) {
			try {
               ...
               /* String str = in.readLine();
				...
				ByteBuffer unwrapEncrypt = ByteBuffer.allocate(audioBlock.length + 4 +4+ 16 + 4);
				...
				unwrapEncrypt.putInt(array_seqNum);
				//next 4 are the seq num of the array
				...
				if (seq_num == interleaveSize) {
					...
					seq_num = 0;
					array_seqNum ^= 1;//alternates sequence number between 0 and 1
					//System.out.println("Alternating......"+array_seqNum);
				}
				//socket.send(packet);
